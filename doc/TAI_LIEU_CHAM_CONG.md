...
|------------|-----------|-----------|----------|
| **Framework UI** | PHP Native (Vanilla PHP) | 8.1+ | Server-side rendering |
...
|------------|-----------|-----------|---------|
| **Framework** | Custom MVC (PHP Native) | - | MVC pattern implementation |
| **Web Server** | NGINX + PHP-FPM | 1.24 + 8.1 | HTTP server |
| **Database Access** | PDO (PHP Data Objects) | Native | Database abstraction |
...
### 3.3. Custom PHP Models (PDO)

```php
// src/Models/PhanCong.php
class PhanCong {
    private $db;
    ...
    public function findByEmployeeAndDate($id_nhanvien, $ngay, $ca) {
    ...
    public function updateCheckIn($id, $gio_vao) {
        $stmt = $this->db->prepare("
            UPDATE phan_cong 
            SET gio_vao = :gio_vao, trang_thai = 1 
            WHERE id = :id
        ");
        return $stmt->execute(['id' => $id, 'gio_vao' => $gio_vao]);
    }
    
    public function updateCheckOut($id, $gio_ra) {
        $stmt = $this->db->prepare("
            UPDATE phan_cong 
            SET gio_ra = :gio_ra, trang_thai = 2 
            WHERE id = :id
        ");
        return $stmt->execute(['id' => $id, 'gio_ra' => $gio_ra]);
    }
}

// src/Models/DangKyKhuonMat.php
class DangKyKhuonMat {
    ...
    public function create($id_nhanvien) {
        $stmt = $this->db->prepare("
            INSERT INTO dang_ky_khuon_mat 
            (id_nhanvien, ngay_dang_ky, trang_thai, created_at, updated_at) 
            VALUES (:id_nhanvien, NOW(), :trang_thai, NOW(), NOW())
        ");
        return $stmt->execute([
            'id_nhanvien' => $id_nhanvien,
    ...
    public function findActiveByEmployee($id_nhanvien) {
        $stmt = $this->db->prepare("
            SELECT * FROM dang_ky_khuon_mat 
            WHERE id_nhanvien = :id_nhanvien 
...
class RapPhim {
    ...
    public function findById($id) {
        $stmt = $this->db->prepare(
            "SELECT * FROM rap_phim WHERE ID = :id"
        );
        $stmt->execute(['id' => $id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
    
    public function getWithDinhVi($id) {
        $stmt = $this->db->prepare("
            SELECT r.*, d.wifi_ten, d.wifi_ip, d.device_id
            FROM rap_phim r
            LEFT JOIN dinh_vi d ON r.ID = d.id_rapphim
            WHERE r.ID = :id
        ");
        $stmt->execute(['id' => $id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
...
## 4. CƠ CHẾ BẢO MẬT KÉP (WIFI NỘI BỘ + ĐỊNH VỊ MẠNG)
...
### 8.1. Thiết bị xác thực WiFi cố định (Anchor Device)
...
### 8.4. Hướng dẫn cấu hình cho kỹ thuật viên

Để hệ thống hoạt động chính xác và tránh xung đột, kỹ thuật viên cần tuân thủ:

**Cấu hình Thiết bị định vị (Anchor Device):**
...
**Cấu hình trên CMS (Web Quản trị):**
